...
$query_tambah = "INSERT INTO pengumuman 
                (judul_pengumuman, isi_pengumuman, tanggal_publikasi, status_publikasi, kategori_pengumuman, gambar_pendukung) 
                VALUES 
                ('$judul', '$isi', '$tanggal', '$status', '$kategori', '$nama_gambar')";

if (mysqli_query($conn, $query_tambah)) {
    header("location: pengumuman_admin.php?pesan=sukses_tambah");
} else {
    echo "Wah gagal bung, ini error MySQL-nya: " . mysqli_error($conn);
}
?>